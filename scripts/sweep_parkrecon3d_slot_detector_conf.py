            ...
            for min_slot_score in min_slot_scores:
                for slot_nms_iou in slot_nms_ious:
                    for orientation_mode in orientation_filter_modes:
                        radii = orientation_neighbor_radii if orientation_mode == "on" else [orientation_neighbor_radii[0]]
                        angle_thresholds = (
                            orientation_angle_thresholds if orientation_mode == "on" else [orientation_angle_thresholds[0]]
                        )
...
def variant_key(variant: dict[str, Any]) -> str:
    ...
    return (
        f"{variant['strategy']}_conf_{variant['slot_conf']:.3f}"
